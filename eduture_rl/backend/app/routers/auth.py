...
def _set_google_oauth_state_cookie(
    response: Response, request: Request, state: str
) -> None:
    response.set_cookie(
        GOOGLE_OAUTH_STATE_COOKIE_NAME,
        state,
        **_google_oauth_state_cookie_options(request),
    )


def _clear_google_oauth_state_cookie(response: Response) -> None:
    response.delete_cookie(
        GOOGLE_OAUTH_STATE_COOKIE_NAME, path=GOOGLE_OAUTH_STATE_COOKIE_PATH
    )
...
def _issue_tokens(
    db: Session, learner: Learner, request: Request | None = None
...
def google_login(request: Request):
    ...
    response = RedirectResponse(
        url=f"{GOOGLE_AUTH_URL}?{query}", status_code=status.HTTP_302_FOUND
    )
...
async def google_callback(
    ...
    try:
        if error:
            log_audit_event(
                ...
                "failure",
                request=request,
                details={"reason": f"google_error:{error}"},
            )
        ...
        log_audit_event(
            ...
            "failure",
            request=request,
            details={"reason": "token_exchange_network_error"},
        )
    ...
    full_name = (
        str(identity.get("name") or email.split("@", 1)[0]).strip()[:100]
        or "Google User"
    )
    ...
    response = RedirectResponse(
        url=settings.google_oauth_frontend_success_url,
    ...
    log_audit_event(
        ...
        request=request,
        actor_id=learner.id,
        details={"email": learner.email},
    )
    return response


@router.post(
    "/register", status_code=status.HTTP_201_CREATED, response_model=ResponseEnvelope
)
def register(
    request: Request,
    response: Response,
    ...
    return ResponseEnvelope(
        data=auth.model_copy(update={"refresh_token": None}).model_dump(
            exclude_none=True
        )
    )
...
def login(
    ...
    if learner is None or not verify_password(payload.password, learner.password_hash):
        log_audit_event(
            ...
            "failure",
            request=request,
            details={"email": payload.email.lower(), "reason": "invalid_credentials"},
        )
        raise HTTPException(
            status_code=401,
            detail={
...
def refresh(
    ...
    if learner is None:
        log_audit_event(
            ...
            "failure",
            request=request,
            details={"reason": "learner_not_found"},
        )
        raise HTTPException(
            status_code=404,
            detail={"code": "NOT_FOUND", "message": "Learner not found", "details": []},
        )
...
def logout(
    ...
    if refresh_token:
        ...
        stored = (
            db.query(RefreshToken)
            .filter(
                RefreshToken.learner_id == current_user.id,
